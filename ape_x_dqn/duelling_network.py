...
class DuellingDQN(torch.nn.Module):
    def __init__(self, state_shape, action_dim):
        ...
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv2d(state_shape[0], 64, 8, stride=4), torch.nn.ReLU()
        )
        self.layer2 = torch.nn.Sequential(
            torch.nn.Conv2d(64, 64, 4, stride=2), torch.nn.ReLU()
        )
        self.layer3 = torch.nn.Sequential(
            torch.nn.Conv2d(64, 64, 3, stride=1), torch.nn.ReLU()
        )
        self.value_stream_layer = torch.nn.Sequential(
            torch.nn.Linear(64 * 7 * 7, 512), torch.nn.ReLU()
        )
        self.advantage_stream_layer = torch.nn.Sequential(
            torch.nn.Linear(64 * 7 * 7, 512), torch.nn.ReLU()
        )
