...
class Learner(object):
    def __init__(self, env_conf, learner_params, shared_state, shared_replay_memory):
        ...
        self.optimizer = torch.optim.RMSprop(
            self.Q.parameters(), lr=0.00025 / 4, weight_decay=0.95, eps=1.5e-7
        )
    ...
    def compute_loss_and_priorities(self, xp_batch):
        ...
        S_t = (
            torch.from_numpy(np.array(n_step_transitions.S_t))
            .float()
            .requires_grad_(True)
        )
        S_tpn = (
            torch.from_numpy(np.array(n_step_transitions.S_tpn))
            .float()
            .requires_grad_(True)
        )
        ...
        with torch.no_grad():
            G_t = (
                rew_t_to_tpB
                + gamma_t_to_tpB
                * self.Q_double(S_tpn)[2]
                .gather(1, torch.argmax(self.Q(S_tpn)[2], 1).view(-1, 1))
                .squeeze()
            )
